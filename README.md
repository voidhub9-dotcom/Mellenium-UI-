...
The floating button distinguishes taps from drags, stays clamped to the viewport, and supports a rounded square or circle. Sliders, dropdowns, color controls, window dragging, resizing, drawers, and confirmation sheets use touch input.
...
Notifications use compact toast cards beneath the mobile safe area, show at most two cards on small viewports, include a close button and countdown line, and restack automatically when one closes.
